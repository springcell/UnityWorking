...
public class TerrainToObjConverter : EditorWindow
{
    ...
    void ConvertTerrainToObj()
    {
       ...
       // StreamWriter sw = new StreamWriter("Terrain.obj");
        string directoryPath = "Assets/TerrainObj";
        Directory.CreateDirectory(directoryPath); // 创建目录，

        StreamWriter sw = new StreamWriter(Path.Combine(directoryPath, "Terrain.obj"));
        ...
        if (textures.Count > 0)
        {
            for (int i = 0; i < textures.Count; i++)
            {
                ...
                string outputPath = Path.Combine(directoryPath, outputPaths[i]);
                ...
                if (texture != null)
                {
                    ...
                    TextureExporter.ExportAsTGA(texture, outputPath);
                }
            }
        }
    }
